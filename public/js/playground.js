...
PLAYGROUND.SoundWebAudioAPI = function (app, audioContext) {
  ...
  this.gainNode.gain.setTargetAtTime(1.0, audioContext.currentTime, 0.015)
...
PLAYGROUND.SoundWebAudioAPI.prototype = {
  ...
  setVolume: function (sound, volume) {
    ...
    return this.gainNode.gain.setTargetAtTime(Math.max(0, volume), this.context.currentTime, 0.015)
...
(function () {
  ...
  cq.Layer.prototype = {
    ...
    colorToMask: function (color, inverted) {
      ...
      for (var i = 0, len = sourcePixels.length; i < len; i += 4) {
        if (sourcePixels[i + 3] > 0) mask.push(!inverted)
    ...
    wrappedText: function (text, x, y, maxWidth, lineHeight) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (textAlign === 'left' || textAlign === 'start') { this.fillText(text, x, oy) } else if (textAlign === 'center') { this.fillText(text, x - width * 0.5 | 0, oy) } else { this.fillText(text, x - width, oy) }
      }
      ...
      if (typeof t === 'object') {
        ...
        if (fill) {
          if (typeof fill === 'string') {
            ...
            if (w - l - r > 0 && h - t - b > 0) { this.drawImage(image, l, t, image.width - r - l, image.height - b - t, x + l, y + t, w - l - r, h - t - b) }
          }
        }
      }
    },
    ...
    setLineDash: function (dash) {
      ...
      this.context.setLineDash(dash || this.noLineDash)
